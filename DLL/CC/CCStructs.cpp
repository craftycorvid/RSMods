...
void CrowdControl::Structs::to_json_request(json& j, const Request& p) {
	j = json{
		{"id", p.id},
		{"code", p.code},
...
void CrowdControl::Structs::from_json_request(const json& j, Request& p) {
	j.at("id").get_to(p.id);
...
/// <param name="p"> - Struct describing the current effect</param>
/// <returns>Serialized JSON for the current effect</returns>
void CrowdControl::Structs::to_json_response(json& j, const Response& p) {
	j = json{
		{"id", p.id},
		{"status", p.status},
		{"code", p.code},
...
/// <param name="p"> - Destination Request struct</param>
/// <returns>Request object describing the current effect</returns>
void CrowdControl::Structs::from_json_response(const json& j, Response& p) {
	j.at("id").get_to(p.id);
